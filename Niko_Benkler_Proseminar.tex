...
Today, the software development process has to face many difficult demands. Fast-changing and unpredictable markets, changing customer requirements \cite{claps2015journey} and rapidly advancing information technologies \cite{olsson2012climbing} require a faster and more flexible process of software development. 
...
CI improves  the effectiveness of a team as much as the software quality \cite{shahin2017continuous}. In other words, CI ensures, that the software is always in a ready to deploy state \cite{claps2015journey}.
...
The main ideas within this section come from \cite{shahin2017continuous}, \cite{schermann2016empirical} and \cite{olsson2012climbing}. They refer to CD as a practice to keep the product "perpetually in a shippable state" using a "fully or at least partially automated deployment pipeline" \cite{schermann2016empirical}. This shows the interchangeability of CDE and CD as they are combining CDE and CD in one term. Besides, they put another practice on top of CD: "Partial Rollouts" (Fig.\ref{Stairway}). 
 \cite{schermann2016empirical} also points out that the adoption of CD does not always has to follow the "stairway to heaven". \cite{taheritanjani2016comparison} presented the "Rugby Process approach which also adopts CD, but in a different way.
...
The core of CI is the continuous integration of code into a shared repository across all teams when developing a software product. Again, a cultural shift is necessary in order to realize CI practices \cite{olsson2012climbing}. Branching strategies like trunk-based development (TBD), the implementation of feature toggle (FT) or the concept of full developer awareness (AWA) facilitate practising CI. The continuous integration of code by many different teams can result in long-living parallel development branches . To reduce this complexity, \cite{schermann2016empirical} proposes trunk-based development, where all teams integrate into one single branch, called master branch or master trunk. What happens to features that are not ready to be deployed, but already integrated into master? The concept of Feature Toggles provides a good solution strategy. In very simple terms, Feature Toggles permit to switch on and off a certain code block. So whenever a feature is ready for production, you just turn it on. Another concept to enable and promote full transparency of the project (e.g build status, test coverage, latest client version or status and progress of features) is called (full) Developer Awareness(AWA). AWA is realized by dashboards, public monitors in the office or chat tools. This simplifies the flow of information and creates transparency.
\\
Fig. \ref{RelCSE} demonstrates, that not only code integration, but also automated builds and test play a huge role in Continuous Practises, especially CI. Adopting "test-driven development" (TDD), "test planning" and "cross-team testing" improves the test phase during CI. TDD is a process in which requirements or user stories are turned into very specific tests. It is divided into 4 different steps: First, add a test based on a required feature; second, run all test and see which test fails; then, write or refactor the code; finally, run the tests again. Repeat those steps in very short development cycles until all test cases are passing. This helps to continuously check the quality of the written code. "Test planning" is a concept to intensify the collaboration between Quality Assurance (QA) and developers according to the DevOps principles. Together, they make a comprehensive list of automated tests. This "liberates QAs from manually testing the majority of the software application for regression bugs" \cite{shahin2017continuous}. In "cross-team testing", tests are performed by a group of developers from another team and vice versa. 
\\
...
\hline
 ...
 \hline
\textbf{Team size} & Big teams, a few dozen poeple & Small teams, 2-8 people & Small teams, 2-8 people \\
  \hline
...
Fig.\ref{Tools} is showing a selection of tools which can be used to build the presented pipeline. They are divided up into the stages of the pipeline. Some of them are expensive enterprise tools, others are Open Source. This seminar paper does not aim to give a detailed explanation or comparison of those tools, as this would be far to complex. 
...
\\
While studying several papers about Continuous Practices in general, it was remarkable how many of them reported about the challenges they had to face while adopting CI,CDE or CD. 
...
\section{Conclusion}
In this seminar paper, I presented a new software development and deployment approach: Continuous Software Engineering. CSE enables developers to deliver software more frequently, reduce time-to-market, obtain customer feedback faster, build the right product or to improve product quality. 
...
Furthermore, CSE requires a shift in culture which is opposed to a general resistance to change mindset. There are neither "out of the box" solutions to implement a continuous deployment pipeline, nor many examples of how organizations exactly adopted Continuous Practices.
\\
I do not claim that CDE is the panacea of software development. But there is always a trade off


 Nevertheless, further research on how to implement Continuous Practices has to be done. New tools have to be developed to fulfil the requirements of CSE and information on how to move applications to 




A possible adoption strategy fo Continuous Practices was given on the basis of the "stairway to heaven" model. 

In this study we...stairway ....
Factors.... aus abstract nehmen

- number of technical and social chalenges 
- shift in culture
- cd should be extremly adaptable to unpredictable changes
-cost
- always atrade off  but eventually it will pay off

-do not claim CD perfect an resolves aevery problem
- benefits ausichtreich
- further research on how to implement CD, tools, transition, 
- Moving apps to CD is challenging, no current information
Architectural conderns, barries and challenges
