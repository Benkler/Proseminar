...
Bla bla wie gut, benefits

\subsection{Organizational Challenges}

\cite{chen2015continuous}
- ream restructure to break down barriers among teams, 
- to litte research done on how to introduce cd

\cite{laukkanen2016bottom}
- gloval distribution of the organization

\cite{shahin2016intersection}
- slow failure recovery
=> You build it, you run it  oder Platzieren der OPS im Büro neben Devs
=> engage ops staff in decision making process at early stage

\cite{shahin2017beyond}
- deployment as business decision which has to be done by management or finacial sectos => give devellopers the authority



\subsection{Process challenges}
\cite{laukkanen2016bottom}
- stage gate process hinder adoption of CD, tight schedules, process overheads because of gate requirements, 
-lack of time to introduce CD
- lack of testing strategy causes duplicate testing--> slow pipeline

\cite{shahin2017continuous}
- security issues in pipeline


\subsection{Technical Challenges}
\cite{chen2015continuous}
- no out of the box solution, own solution => costl

\cite{claps2015journey}
- Infrastructure requires proper hardware
- seamless upgrade (no downtime) => use two parallel running systems
- bugs are mroe likely to slip through process => outweigthed by CD, fixing b ugs quickly

\cite{laukkanen2016bottom}
-unstable slow tests, insufficients testing environments
- multiple braches need own hardware ressource (pipeline for each branch)

\cite{olsson2012climbing}
-variety of available test tools is problematic /main issue"!""""!!! => fully automated testing infrastructure needs to be implemented
- lack of transpareny => full develloper awareness
-different network confuguration at customer site

\cite{shahin2017beyond}
- lack of fully automated user acceptance test because a) too much effort with low gain, b) tool limitations, c)lack of automation skill
- maual quality tasks such as quality checks are sometime unavoidabel
...
\cite{laukkanen2016bottom}
- no trust in build and test results (confidence)

\cite{claps2015journey}
- pressure for developper, code always need to be ready to deploy => improve communication
- lack of moticvation => ensure top managers are pushing
- team coordination requires extra effort => outhwigted by benefits(pruductivity steigerung

\cite{shahin2017beyond}
- demotivated customer, they need to be involved, this is work and cost => present benefits
